...
import { IFileStore, FileNotFoundError, FileTooLargeError, type FileStoreOptions } from './file-store.js';
...
export class FileStore extends Disposable implements IFileStore {
  ...
  async save(
    source: Readable,
    filename: string,
    options: import('./file-store.js').SaveOptions = {},
  ): Promise<FileMeta> {
  ...
  async get(fileId: string): Promise<import('./file-store.js').GetResult> {
    await this.ensureIndex();
