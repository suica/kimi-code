...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'kimi-daemon-files-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'kimi-daemon-files-home-'));
});
...
function appOf(r: RunningDaemon): FastifyAppLike {
...
describe('POST /api/v1/files (W12.2 / Chain 15)', () => {
  ...
  it('survives daemon restart: index.json persists upload across instances', async () => {
    // Upload under daemon #1.
    ...
    // Restart daemon (same homeDir, fresh process state).
    await r.close();
    daemon = undefined;
    r = await bootDaemon();
