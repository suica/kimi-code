...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'kimi-daemon-auth-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'kimi-daemon-auth-home-'));
});
...
/**
 * Seed `<bridgeHome>/config.toml` BEFORE daemon boot. Path layout matches
