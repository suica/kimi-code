 ...
 *   2. client kill mid-stream → daemon log "client aborted" — no leak
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'kimi-daemon-fs-download-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'kimi-daemon-fs-download-home-'));
...
describe('GET /api/v1/sessions/{sid}/fs/{path}:download (W11.3)', () => {
  ...
  it('path with `..` → HTTP 200 + envelope code 41304', async () => {
    ...
    // before the route handler sees it; the daemon's path-safety guard
