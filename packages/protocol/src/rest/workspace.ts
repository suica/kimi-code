/**
 * Workspace REST endpoint schemas.
 *
 ...
 *
 * POST is idempotent on `root` — the daemon resolves `realpath(root)` and
 * keys the workspace by `encodeWorkDirKey(realRoot)`. Second call with the
 * same root touches `last_opened_at` and returns the same Workspace.
 *
 * Errors:
