 ...
 *      (and ISO time normalization on `started_at` works).
 *   2. Negative parses — each required field's absence is caught.
 *   3. Capability literal narrowing — non-`true` boolean fails.
 *   4. ISO time normalization on `started_at` produces canonical UTC `Z`.
 */
