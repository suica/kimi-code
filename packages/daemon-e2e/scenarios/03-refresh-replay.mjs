 ...
 *
 * Usage:
 *   DAEMON_URL=http://127.0.0.1:7878 npx tsx scenarios/03-refresh-replay.mjs
 *
 * Exit codes:
 *   0  — pass
...
import { DaemonClient, WsClient } from '../src/index.ts';
...
async function openSocketWithHello({ sid, lastSeq }) {
  ...
  const payload = { client_id: `scenario-03-${process.pid}`, subscriptions: [sid] };
...
async function main() {
  ...
  // ── Phase 0 ─────────────────────────────────────────────────────────────
  const health = await fetchEnvelope(`${DAEMON_URL}${API_PREFIX}/healthz`);
  assert.equal(health.ok, true, 'healthz did not return ok=true');

  const meta = await fetchEnvelope(`${DAEMON_URL}${API_PREFIX}/meta`);
  ...
  const auth = await fetchEnvelope(`${DAEMON_URL}${API_PREFIX}/auth`);
  ...
  try {
    ...
    const meta2 = await fetchEnvelope(`${DAEMON_URL}${API_PREFIX}/meta`);
    ...
    const ackPayloadA = caughtUp.ack.payload ?? {};
    assert.deepEqual(
    ...
    const ackPayloadB = replay.ack.payload ?? {};
    assert.deepEqual(
    ...
    const seqs = replay.replayed.map((f) => f.seq).filter((n) => typeof n === 'number');
    assert.equal(
      Math.min(...seqs),
      1,
    ...
    const tasks = await fetchEnvelope(
