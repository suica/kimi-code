 ...
 *
 * Usage:
 *   DAEMON_URL=http://127.0.0.1:7878 npx tsx scenarios/05-workspace.mjs
 *
 * Exit codes:
 *   0  — pass
...
import { DaemonClient } from '../src/index.ts';
...
async function main() {
  ...
  try {
    ...
    assert.equal(filtered.items[0].id, sid, 'session id round-trips through the filter');
    ...
    if (process.env.DAEMON_AUTH !== 'skip') {
      ...
      try {
        ...
        console.log(`▶ prompt round-trip skipped (likely no DAEMON_AUTH): ${err}`);
      }
    } else {
