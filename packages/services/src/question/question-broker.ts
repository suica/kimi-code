/**
 * Question broker interface + protocol adapter (W8.2 / Chain 6).
 *
 * **Broker interface** (`IQuestionBroker`): Reverse-RPC broker that routes
 * `QuestionRequest`s coming out of `KimiCore` to a waiter (web client over
 * WS in P1.x, mock handler in tests) and resolves the promise when the
 * response arrives — or `dismiss()`-es it if the user closes the panel
 * (SCHEMAS.md §6.3).
 *
